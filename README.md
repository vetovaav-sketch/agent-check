...
- Агент проверяет **по одному требованию за раз**.
- Сначала ищет релевантные чанки кода, потом отправляет их в модель.
- В каждом evidence всегда сохраняются `file_path`, `line_start`, `line_end`.
- Без доказательств агент не делает уверенных выводов (`ok/wrong`).
- Отчет формируется на русском языке.
